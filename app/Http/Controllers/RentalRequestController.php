   ...
   public function store(Request $request)
    {
        ...
        if (empty($data['room_type_id']) && empty($data['kamar_id'])) {
            return back()
                ->withErrors(['room_type_id' => 'Pilih tipe kamar atau langsung pilih kamar untuk dipesan.'])
                ->withInput();
        }

        // jika user memilih kamar spesifik
        if (!empty($data['kamar_id'])) {
            $kamar = \App\Models\Kamar::with('roomType')->findOrFail($data['kamar_id']);

            // 🔴 kamar belum punya room type (data rusak)
            if (!$kamar->room_type_id) {
                return back()
                    ->withErrors(['kamar_id' => 'Kamar ini belum memiliki tipe kamar.'])
                    ->withInput();
            }

            // ✅ set room_type_id otomatis dari kamar
            $data['room_type_id'] = $kamar->room_type_id;
            ...
            if ($conflict) {
                return back()
                    ->withErrors(['kamar_id' => 'Maaf, kamar ini sudah tidak tersedia pada rentang tanggal yang dipilih.'])
                    ->withInput();
            }
        }
        ...
        return redirect()
            ->route('user.rental_requests.index')
            ->with('success', 'Permintaan sewa dikirim. Silakan tunggu konfirmasi dari pemilik.');
    }
